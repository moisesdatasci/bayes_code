...
class Search():
    ...
    def __init__(self, name):
        ...
        self.sep1_range = (0.2, 0.4) # Hard
        self.sep2_range = (0.7, 0.9) # Easy
        self.sep3_range = (0.4, 0.6) # Medium
...
def main():
    ...
    while True:
        if turnos_restantes == 0:
            ...
            # Mostrar la ubicación final
            cv.circle(app.img, (sailor_x, sailor_y), 3, (0, 0, 255), -1)
            cv.putText(app.img, 'AQUI ESTABA', (sailor_x + 5, sailor_y),
                       cv.FONT_HERSHEY_PLAIN, 1, (0, 0, 255))
            cv.imshow('Search Area', app.img)
            cv.waitKey(0)
            break

        print(f"\n--- Búsquedas restantes: {turnos_restantes} ---")
